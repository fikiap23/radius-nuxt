...
const description = computed(() =>
	status.value === "loading"
		? "Exchanging your Google authorization with the server."
		: "We could not finish Google sign-in.",
);
...
onMounted(async () => {
	...
	if (!code) {
		status.value = "error";
		errorMessage.value = "Missing authorization code from Google.";
		return;
	}

	const result = await authStore.completeGoogleCallback(code);
